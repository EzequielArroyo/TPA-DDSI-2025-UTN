...
public class DesastresApiClient {
  ...
  public Flux<HechoDTO> obtenerTodosLosHechos() {
    Mono<Void> ensureToken = (apiKey == null) ? iniciarSesion() : Mono.empty();

    return ensureToken.thenMany(
        obtenerPagina("/api/desastres?page=1&per_page=100")
            .expand(response -> {
              if (response.getNext_page_url() != null) {
                return obtenerPagina(response.getNext_page_url() + "&per_page=100");
              } else {
                return Mono.empty();
              }
            })
            .flatMapIterable(PaginatedResponseDTO::getData)
    );
  }
}
