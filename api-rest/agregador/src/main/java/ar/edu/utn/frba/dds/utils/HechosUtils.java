...
public class HechosUtils {
  public static Boolean sonIguales(Hecho h1, Hecho h2) {
    return h1.getTitulo().equalsIgnoreCase(h2.getTitulo())
        && h1.getDescripcion().equalsIgnoreCase(h2.getDescripcion())
        && h1.getCategoria().equals(h2.getCategoria())
        && h1.getUbicacion().equals(h2.getUbicacion())
        && h1.getFechaAcontecimiento().equals(h2.getFechaAcontecimiento());
  }
  public static void fusionarOrigenes(Hecho existente, Hecho nuevo) {
    for (Origen origenNuevo : nuevo.getOrigen()) {
      boolean yaExiste = existente.getOrigen().stream()
          .anyMatch(o -> o.getTipoFuente().equals(origenNuevo.getTipoFuente())
              && o.getRaiz().equals(origenNuevo.getRaiz()));
      if (!yaExiste) {
        existente.getOrigen().add(origenNuevo);
      }
    }
  }
