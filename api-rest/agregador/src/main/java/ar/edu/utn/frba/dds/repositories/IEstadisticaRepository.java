...
public interface IEstadisticaRepository extends JpaRepository<Hecho, Long> {
  ...
  @Query("SELECT new ar.edu.utn.frba.dds.entities.dto.input.estadistica.ProvinciaCantidadDTO(h.ubicacion.provincia, COUNT(h.id)) " +
  ...
  @Query("SELECT COUNT(s) FROM SolicitudDeEliminacion s WHERE s.esSpam = true")
  Long contarPorEstado(@Param("estado") Estado estado);
