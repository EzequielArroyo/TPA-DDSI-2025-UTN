...
public class EstadisticaService implements IEstadisticaService {
  ...
  public List<ProvinciaCantidadDTO> obtenerTopProvinciasPorColecciones() {
    ...
    for (Coleccion coleccion : colecciones) {
       topProvincias.add(estadisticaRepository.contarHechosPorProvinciaDesc(coleccion.getId(), PageRequest.of(0,1)).getContent().get(0));
    }
    return topProvincias;
  }

  public CategoriaCantidadDTO obtenerTopCategoria() {
    return estadisticaRepository.contarHechosPorCategoriaDesc(PageRequest.of(0,1)).getContent().get(0);
  }
  ...
  public List<ProvinciaCantidadCategoriaDTO> obtenerTopProvinciaPorCategoria() {
    ...
    for (Categoria categoria : categorias) {
      topProvincias.add(estadisticaRepository.contarCategoriasPorProvinciaDesc(categoria.getId(),PageRequest.of(0,1)).getContent().get(0));
    }
  ...
  public List<HoraCantidadCategoriaDTO> obtenerHoraPicoPorCategoria() {
    ...
    for (Categoria categoria : categorias) {
      HoraCantidadCategoriaDTO horaPicoCategoria = estadisticaRepository.contarHorasDeAcontecimientoPorCategoria(categoria.getId(), PageRequest.of(0,1)).getContent().get(0);
      horaPico.add(horaPicoCategoria);
    }
    return horaPico;
  }

  public Long obtenerCantidadSpam() {
    return estadisticaRepository.contarPorEstado(Estado.DENEGADA);
  }
